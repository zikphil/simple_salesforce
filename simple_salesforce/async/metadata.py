...
class SfdcMetadataApi(object):
    ...
    def _session_header(self):
        return self._client.get_element('ns0:SessionHeader')(sessionId=self.transport.session_id)

    # pylint: disable=R0913
    def __init__(self, sandbox, transport_instance):
        ...
        self._service = self._client.create_service(f"{{{self._XML_NAMESPACES['mt']}}}MetadataBinding", self.transport.metadata_url)
    ...
    def _retrieve_deploy_result(self, async_process_id, **kwargs):
        ...
        res = self.transport.call(
            endpoint='deployRequest/' + async_process_id,
            method='POST',
            api='metadata',
            additional_headers=headers,
            data=mt_request
